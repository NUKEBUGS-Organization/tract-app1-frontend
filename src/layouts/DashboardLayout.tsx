...
function getPrimaryAction(title: string) {
  ...
  if (normalizedTitle.includes("admin")) {
    return { label: "Review Queue", path: "/verifications" };
  }
  if (normalizedTitle.includes("partner")) {
    return { label: "View Properties", path: "/properties" };
  }
  if (normalizedTitle.includes("licensed")) {
    return { label: "View Deals", path: "/deals" };
  }
  return { label: "Create New Listing", path: "/list-property" };
}
...
function filterListings(listings: any[], searchValue: string) {
  ...
  return listings
    .filter((listing) => getListingSearchText(listing).includes(normalizedSearch))
    .slice(0, 6);
}
...
function DashboardLayout({
  ...
  const profileMatchesCurrentUser =
    ...
    (authUser?.email && profileUser?.email && authUser.email === profileUser.email);

  const displayUser = profileMatchesCurrentUser ? profileUser || authUser : authUser;
  ...
  const { data: dashboardData, isFetching: isFetchingListings } =
    useGetListingsDashboardQuery(undefined, { skip: !showPropertySearch });
  ...
  const closeSearchDropdown = () => setIsSearchFocused(false);
  ...
  const titleLabel = isDark ? "text-white/40" : "text-[var(--color-text-muted)]";
  const titleHeading = isDark ? "text-white" : "text-[var(--color-primary)]";
  ...
  const chevronColor = isDark ? "text-white/50" : "text-[var(--color-text-muted)]";

  /* Profile dropdown: light in light mode, dark card in dark mode */
  const profileDropdownBg = isDark
    ? "bg-[var(--color-dark-card)] border-white/10"
    : "bg-white border-[var(--color-border-light)]";

  const profileDropdownNameColor = isDark ? "text-white" : "text-[var(--color-primary)]";
  const profileDropdownSubColor = isDark ? "text-white/40" : "text-[var(--color-text-muted)]";
  const profileDropdownItemColor = isDark
    ? "text-white/80 hover:bg-white/8"
    : "text-[var(--color-text-main)] hover:bg-[var(--color-bg-soft)]";
  const profileDropdownIconColor = isDark ? "text-[var(--color-secondary)]" : "text-[var(--color-primary)]";
  const profileDropdownBorderColor = isDark ? "border-white/10" : "border-[var(--color-border-light)]";
          ...
          {isMobileMenuOpen && (
            <div className="fixed inset-0 z-40 lg:hidden">
              ...
              <aside className="relative z-50 flex h-full w-[280px] flex-col bg-[var(--color-primary-dark)] text-white shadow-2xl">
                <DashboardSidebar navItems={navItems} onNavigate={() => setIsMobileMenuOpen(false)} />
              </aside>
            </div>
          )}
            ...
            >
              ...
              <div className="flex min-w-0 items-center gap-4 lg:gap-6">
                ...
                {!isMobileMenuOpen && (
                  <div className="flex shrink-0 items-center gap-2 lg:hidden">
                    <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--color-secondary)]/30 bg-white/90 shadow-sm">
                      <img src="/tract-logo.png" alt="TRACT logo" className="h-6 w-6 object-contain" />
                    </div>
                ...
                <div className="hidden min-w-0 lg:block">
                  <p className={`text-[10px] font-semibold uppercase tracking-[0.25em] sm:text-xs ${titleLabel}`}>
                    {title}
                  </p>
                  <h2 className={`mt-1 truncate font-serif text-xl font-black leading-tight sm:text-2xl lg:text-3xl ${titleHeading}`}>
              ...
              <div className="flex shrink-0 items-center gap-4 lg:gap-6">
                {showPropertySearch && (
                  <div className="relative hidden w-[280px] md:block xl:w-[320px]">
                    <div className={`flex h-11 items-center gap-3 rounded-none px-4 ${isDark ? "bg-white/10" : "bg-white/70"}`}>
                      ...
                      <input
                        ...
                        className={`w-full bg-transparent text-sm outline-none placeholder:text-[var(--color-text-muted)] ${isDark ? "text-white" : "text-[var(--color-text-main)]"}`}
                      />
                      {searchValue && (
                        <button type="button" onClick={handleClearSearch} className="shrink-0 rounded-full p-1 text-[var(--color-text-muted)] transition hover:bg-black/5 hover:text-[var(--color-primary)]" aria-label="Clear search">
                          <X className="h-4 w-4" />
                        </button>
                      )}
                    </div>
                    {shouldShowDropdown && (
                      <div className="absolute left-0 top-[52px] z-50 w-full overflow-hidden rounded-2xl border border-[var(--color-border-light)] bg-white shadow-2xl">
                        <div className="border-b border-[var(--color-border-light)] px-4 py-3">
                          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-muted)]">Matching Listings</p>
                        </div>
                        {isFetchingListings ? (
                          <div className="px-4 py-5 text-center text-xs font-semibold text-[var(--color-text-muted)]">Searching listings...</div>
                        ) : searchResults.length === 0 ? (
                          <div className="px-4 py-5 text-center">
                            <p className="text-sm font-bold text-[var(--color-text-main)]">No matching listing found.</p>
                            <p className="mt-1 text-xs text-[var(--color-text-muted)]">Try address, state, ZIP code, property type, or status.</p>
                          </div>
                        ) : (
                          <div className="max-h-[360px] overflow-y-auto py-2">
                            {searchResults.map((listing: any) => {
                              ...
                              return (
                                <Link key={id} to={`/listings/${id}`} onMouseDown={(e) => e.preventDefault()} onClick={closeSearchDropdown} className="block px-4 py-3 transition hover:bg-[var(--color-bg-soft)]">
                                  <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                      <p className="truncate text-sm font-black text-[var(--color-primary)]">{label}</p>
                                      <p className="mt-1 truncate text-xs text-[var(--color-text-muted)]">{listing?.property_type || "Property"} · {formatMoney(listing?.market_price)}</p>
                                    </div>
                                    <span className="shrink-0 rounded-full border border-[var(--color-border-light)] px-2 py-1 text-[9px] font-black uppercase tracking-wider text-[var(--color-text-muted)]">{formatStatus(listing?.status)}</span>
                                  </div>
                                </Link>
                              );
                            })}
                        ...
                        {searchResults.length > 0 && (
                          <div className="border-t border-[var(--color-border-light)] px-4 py-3">
                            <p className="text-[10px] font-semibold text-[var(--color-text-muted)]">Click a listing to open its details page.</p>
                          </div>
                        )}
                      </div>
                    )}
                ...
                {showThemeToggle && onToggleTheme && (
                  <button
                    ...
                    className={`
                    relative flex h-11 w-[88px] items-center rounded-full border transition-all duration-300
                    ${isDark
                  ...
                  >
                    ...
                    <span
                      className={`
                      absolute z-10 flex h-8 w-8 items-center justify-center rounded-full shadow-md transition-all duration-300
                      ${isDark
                    ...
                    {/* Labels */}
                    <span className={`absolute left-[40px] z-10 text-[9px] font-black uppercase tracking-widest transition-opacity duration-200 ${isDark ? "opacity-100 text-white/30" : "opacity-0"}`}>
                      Day
                    </span>
                    <span className={`absolute left-[12px] z-10 text-[9px] font-black uppercase tracking-widest transition-opacity duration-200 ${isLight ? "opacity-100 text-[var(--color-text-muted)]" : "opacity-0"}`}>
                      Ngt
                    </span>
                  </button>
                )}
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronDown className={`hidden h-4 w-4 transition md:block ${chevronColor} ${isProfileMenuOpen ? "rotate-180" : ""}`} />
                  </button>

                  {isProfileMenuOpen && (
                    <div className={`absolute right-0 top-[56px] z-50 w-72 overflow-hidden rounded-2xl border shadow-2xl ${profileDropdownBg}`}>
                      <div className={`border-b px-5 py-4 ${profileDropdownBorderColor}`}>
                        <p className={`truncate text-sm font-black ${profileDropdownNameColor}`}>{displayName}</p>
                        <p className={`mt-0.5 truncate text-xs ${profileDropdownSubColor}`}>Profile & account settings</p>
                      </div>
                      ...
                      <div className="py-2">
                        ...
                        >
                          <UserCircle className={`h-4 w-4 ${profileDropdownIconColor}`} />
                          Profile & Settings
                        </Link>
                        ...
                        <Link
                          ...
                          className={`flex items-center gap-3 px-5 py-3 text-sm font-bold transition ${profileDropdownItemColor}`}
                        >
                          <ShieldCheck className={`h-4 w-4 ${profileDropdownIconColor}`} />
                          KYC Verification
                        </Link>
                        ...
                        {isPartner && (
                          <Link
                            ...
                            className={`flex items-center gap-3 px-5 py-3 text-sm font-bold transition ${profileDropdownItemColor}`}
                          >
                            <FileText className={`h-4 w-4 ${profileDropdownIconColor}`} />
                            Proof of Activity
                          </Link>
                        )}
                      </div>
